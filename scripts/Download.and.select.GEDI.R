...
# leaflet() %>%
#   addCircleMarkers(clipped.filt$lon_lowestmode,
#                    clipped.filt$lat_lowestmode,
#                    radius = 0.1,weight = 0.1,
#                    opacity = 0.2,
#                    color = "red")  %>%
...
# clipped.filt.avg <- clipped.filt %>%
#   mutate(lon.m = round(lon_lowestmode,digits = 2),
#          lat.m = round(lat_lowestmode,digits = 2)) %>%
#   group_by(lon.m,lat.m) %>%
#   summarise(agbd.m = mean(agbd,na.rm = TRUE),
#             agbd.sd = sd(agbd,na.rm = TRUE),
#             .groups = "keep")
#
#
# ggplot(data = clipped.filt.avg) +
#   geom_tile(aes(x = lon.m, y = lat.m,
#                  fill = agbd.m)) +
#   scale_fill_gradient(limits = c(0,500),low = "white",high = "darkgreen",
#                        oob = scales::squish) +
#   labs(x = "", y = "") +
#   theme_bw() +
#   guides(alpha = "none")
...
l4_plotagb(clipped.filt,
           beam_id="all",type = "both",
           n=100,h=c(100,100))

l4_plotprofile(clipped %>%
                 filter(source == source[1]),beam_id="all",
               tct = 50)
