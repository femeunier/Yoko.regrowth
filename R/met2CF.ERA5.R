                        ...
                        type = "reanalysis")
{
  ...
  ensemblesN <- seq(1, length(out.xts))
  ...
  out.new <- ensemblesN %>%
    purrr::map(function(ensi) {
      tryCatch({
        ...
        } else{ # ensemble member
          ens[, "ssrd"] <- ens[, "ssrd"] / (3 * 3600)
          ens[, "strd"] <- ens[, "strd"] / (3 * 3600)
          ens[, "tp"] <-
            ens[, "tp"] * 1000 / 3 # divided by 3 because we have 3 hours data --> mm/h
        }
      ...
      #adding humidity
      xts::merge.xts(ens[,-c(8)], (specific_humidity)) %>%
        `colnames<-`(
          c(
            "air_temperature",
            "air_pressure",
